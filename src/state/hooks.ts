...
import { Toast, toastTypes } from '@cheeseswapfinance/uikit'
...
export const usePoolFromPid = (cnftId): Pool => {
  const pool = useSelector((state: State) => state.pools.data.find((p) => p.cnftId === cnftId))
  return pool
}
