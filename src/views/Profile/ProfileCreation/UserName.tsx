...
const minimumCnftToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumCnftRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumCnftRequired = useHasCnftBalance(minimumCnftToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumCnftRequired={minimumCnftRequired}
      allowance={allowance}
    />,
    false,
  )
  ...
  const handleConfirm = async () => {
    try {
      ...
      const signature = library?.htSign
        ? (await library.htSign(account, userName))?.signature
    ...
    <>
      ...
      {!hasMinimumCnftRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} CNFT is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
