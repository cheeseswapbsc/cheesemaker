...
import { getCnftYieldRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const cnftYieldRabbitsContract = getCnftYieldRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await cnftYieldRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          ...
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = cnftYieldRabbitsContract.methods
