...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@cheeseswapfinance/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberCnftToReactivate } = useGetProfileCosts()
  const TranslateString = useI18n()
  const cnftYieldProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = () => {
    cnftYieldProfileContract.methods
      .pauseProfile()
      .send({ from: account })
      .on('sending', () => {
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {TranslateString(999, `Cost to reactivate in future: ${getBalanceNumber(numberCnftToReactivate)} CNFT`)}
      </Text>
