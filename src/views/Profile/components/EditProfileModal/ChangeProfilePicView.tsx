...
import { Button, InjectedModalProps, Skeleton, Text } from '@cheeseswapfinance/uikit'
...
import { useCnftYieldRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getCnftYieldProfileAddress, getCnftYieldRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const cnftYieldRabbitsContract = useCnftYieldRabbits()
  ...
  const {
    ...
    onApprove: () => {
      return cnftYieldRabbitsContract.methods.approve(getCnftYieldProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getCnftYieldRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getCnftYieldRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a CnftYield Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
