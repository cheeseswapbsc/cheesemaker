...
import { useCnft, useBunnyFactory } from 'hooks/useContract'
...
const minimumCnftBalanceToMint = new BigNumber(MINT_COST).multipliedBy(new BigNumber(10).pow(18))

const Mint: React.FC = () => {
  const [bunnyId, setBunnyId] = useState(null)
  const { actions, minimumCnftRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const cnftContract = useCnft()
  const bunnyFactoryContract = useBunnyFactory()
  const TranslateString = useI18n()
  const hasMinimumCnftRequired = useHasCnftBalance(minimumCnftBalanceToMint)
  const {
    isApproving,
    isApproved,
    isConfirmed,
    ...
    onRequiresApproval: async () => {
      ...
      try {
        const response = await cnftContract.methods.allowance(account, bunnyFactoryContract.options.address).call()
        const currentAllowance = new BigNumber(response)
        return currentAllowance.gte(minimumCnftRequired)
      } catch (error) {
        return false
      }
    },
    onApprove: () => {
      return cnftContract.methods
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {TranslateString(790, 'You can change your profile pic later if you get another approved CnftYield Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {TranslateString(999, `Cost: ${MINT_COST} CNFT`, { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumCnftRequired}
              >
          ...
          {!hasMinimumCnftRequired && (
            <Text color="failure" mb="16px">
              {TranslateString(1098, `A minimum of ${MINT_COST} CNFT is required`)}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={bunnyId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumCnftRequired}
